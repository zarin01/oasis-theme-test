 ...
 * Oasis Pro.
 *
 * This file adds the required WooCommerce setup functions to the Oasis Pro Theme.
 *
 * @package oasis
 * @author  StudioPress
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'oasis_woocommerce_breakpoint' );
/**
 * Modifies the WooCommerce breakpoints.
 *
 * @since 1.1.0
 */
function oasis_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_default_products_per_page', 'oasis_default_products_per_page' );
/**
 ...
 */
function oasis_default_products_per_page() {
	return 8;
}

add_filter( 'woocommerce_pagination_args', 'oasis_woocommerce_pagination' );
/**
 ...
 */
function oasis_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'oasis-pro' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'oasis-pro' ) );

	return $args;

}

add_action( 'after_switch_theme', 'oasis_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 1.1.0
 */
function oasis_woocommerce_image_dimensions_after_theme_setup() {
	...
	oasis_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'oasis_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function oasis_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 1.1.0
 */
function oasis_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'oasis_gallery_image_thumbnail' );
/**
 ...
 */
function oasis_gallery_image_thumbnail( $size ) {
